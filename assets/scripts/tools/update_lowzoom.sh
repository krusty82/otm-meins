...
psql -d gis -c "CREATE OR REPLACE VIEW lowzoom_bayllabel    AS SELECT ST_LineMerge(longway) AS way,get_localized_placename(name,\"name:de\",int_name,\"name:en\",true) as name,'bayaxis'::text    AS label,len*len/10 as way_area FROM (SELECT ST_Collect(way) AS longway,SUM(ST_Length(way)) AS len,MAX(name) AS name FROM planet_osm_line WHERE \"natural\"='bay' AND name is NOT NULL GROUP BY osm_id) AS t;"
